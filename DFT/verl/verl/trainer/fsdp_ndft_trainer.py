...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        loss_mask = batch.pop("loss_mask")[:, :-1]
        rollout_length = loss_mask.sum(dim=-1, keepdim=True)
        loss_mask = loss_mask / (rollout_length + 1e-8)

        loss_mask = loss_mask.reshape(-1).to(self.device_name)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            valid_token_this_rank = input_ids.shape[0]
            ...
            original_loss = torch.sum(original_loss) / valid_token_this_rank * dp_size
...
def main(config):
    print("Running normalized dynamic fine-tuning (NDFT).")
    run_sft(config)
