...
class RayPPOTrainer:
    ...
    def _save_checkpoint(self):
        ...
        if self.config.trainer.save_mode == "last":
            try:
            ...
            last_global_step_folder = os.path.join(
                self.config.trainer.default_local_dir, f"global_step_{last_saved_step}"
            )

            ## remove the previous directory if it exists
            if os.path.exists(last_global_step_folder):
                print(f"Removing previous checkpoint directory: {last_global_step_folder}")
                import shutil
                shutil.rmtree(last_global_step_folder)
