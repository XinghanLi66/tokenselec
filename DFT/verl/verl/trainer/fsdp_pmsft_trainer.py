...
from verl.utils.dataset import PMSFTDataset
...
class FSDPSFTTrainer:
    ...
    def _normalize_config_bsz(self):
        ...
        assert self.config.data.train_batch_size % self.config.data.micro_batch_size_per_gpu == 0, \
            f"Global batch size {self.config.data.train_batch_size} is not divisible by micro batch size {self.config.data.micro_batch_size_per_gpu}"
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        rewards = batch["reward"].to(self.device_name)
        ...
        ## normalize: divide each row of loss by rollout length
        loss_mask = batch.pop("loss_mask")[:, :-1].to(self.device_name)  
        rollout_length = loss_mask.sum(dim=-1, keepdim=True)
        loss_mask = loss_mask * rewards / (rollout_length + 1e-8) 
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                #     print(f"loss_mask shape: {loss_mask.shape}")  ## torch.Size([8188])
                #     print(f"loss shape: {loss.shape}")  ## torch.Size([8188]), 2047 * 4
                #     print(f"attention_mask shape: {attention_mask.shape}")  ## torch.Size([4, 2048])
                #     print(f"input_ids shape: {input_ids.shape}")  ## torch.Size([4, 2048])
                #     print(f"position_ids shape: {position_ids.shape}")  ## torch.Size([4, 2048])
                #     ## print the whole loss_mask and attention_mask
                #     print(f"loss_mask: {loss_mask}")  ## first 0 (prompt) then 1 (resp) then 0 (pad)
                #     print(f"attention_mask: {attention_mask}")  ## first 1 (prompt+resp) and then 0 (pad)
                #     print(f"position_ids: {position_ids}")  ## 926. 389. 317. 285, (0,1,2,...,285,285,...285)
            ...
            # valid_token_this_rank = torch.sum(loss_mask)  ## it is mini batch size now.
            batch_size_this_rank = input_ids.shape[0]

            if self.config.data.balance_dp_token:
                raise NotImplementedError("balance dp token not supported yet.")
                torch.distributed.all_reduce(valid_token_this_rank)
                dp_size = self.ulysses_device_mesh.size("dp") if use_sp else torch.distributed.get_world_size()
            else:
                dp_size = 1

            loss = torch.sum(loss) / batch_size_this_rank * dp_size

            original_loss = torch.sum(original_loss) / batch_size_this_rank * dp_size
    ...
    def fit(self):
        ...
        ## for debug
        # print("len of train dataloader:", len(self.train_dataloader))
...
def main(config):
    print("Running positive/minus sft (PMSFT).")
    run_sft(config)
...
def create_sft_dataset(data_paths, data_config, tokenizer):
    ...
    # Then check if multi-turn dataset should be used
        ...
        raise NotImplementedError("multiturn dataset not supported yet.")
        dataset_cls = MultiTurnSFTDataset
    # Default to single-turn dataset
    else:
        dataset_cls = PMSFTDataset
