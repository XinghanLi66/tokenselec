...
def parse_args():
    ...
    # Legacy outputs (used only if split=train_valid and output_dir is not provided)
    ...
    # New unified output directory (preferred): saves correct/incorrect without train/valid split
    parser.add_argument("--output_dir", type=str, default=None, help="Directory to save outputs (correct/incorrect). If provided and split=none, writes correct/incorrect parquet/csv here.")
    ...
    # Output split behavior
    parser.add_argument("--split", type=str, choices=["none", "train_valid"], default="none",
                        help="If 'none', write correct/incorrect sets; if 'train_valid', keep legacy train/valid outputs.")
...
def main():
    ...
    ## Under main process, filter correct vs incorrect and save according to split mode
    if acc.is_main_process:
        valid_texts  = keep_correct_answers(gen_texts, answer)
        # Partition into correct (valid_texts) and incorrect (complement), preserving multiplicity
        ...
        if args.split == "none":
            # Save full correct/incorrect without train/valid split
            target_dir = output_dir if output_dir is not None else os.path.dirname(parquet_out)
            os.makedirs(target_dir, exist_ok=True)
            correct_df = pd.DataFrame({"prompt": [formatted_prompt] * len(correct_list),
                                       "response": correct_list})
            incorrect_df = pd.DataFrame({"prompt": [formatted_prompt] * len(incorrect_list),
                                         "response": incorrect_list})
            correct_df.to_parquet(os.path.join(target_dir, "correct.parquet"), index=False)
            correct_df.to_csv(os.path.join(target_dir, "correct.csv"), index=False)
            incorrect_df.to_parquet(os.path.join(target_dir, "incorrect.parquet"), index=False)
            incorrect_df.to_csv(os.path.join(target_dir, "incorrect.csv"), index=False)
            acc.print(f"Saved correct={len(correct_df)} and incorrect={len(incorrect_df)} to {target_dir}.")
        else:
            # Legacy behavior: save train/valid from correct_list only
            assert len(correct_list) >= args.n_train, \
                f"Not enough valid samples: {len(correct_list)} found, but {args.n_train} are required."
            acc.print(f"Generated {len(gen_texts)} samples; {len(correct_list)} are approximately equal to the answer '{answer}'. "
                      f"Saving {args.n_train} of them to {parquet_out} and {csv_out}. "
                      f"{len(correct_list) - args.n_train} valid samples will be saved to {parquet_out_val} and {csv_out_valid}.")
            out_df = pd.DataFrame({"prompt": [formatted_prompt] * args.n_train,
                                   "response": correct_list[:args.n_train]})
            out_df_val = pd.DataFrame({"prompt": [formatted_prompt] * (len(correct_list) - args.n_train),
                                       "response": correct_list[args.n_train:]})
            ...
            acc.print(f"Saved {args.n_train} valid samples to {parquet_out} and {csv_out}, and {len(correct_list) - args.n_train} valid samples to {parquet_out_val} and {csv_out_valid}.")
